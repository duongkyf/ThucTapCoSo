...
const getSeats = async (req, res) => {
  try {
    const { date } = req.query; // ?date=2026-04-11 — cần cho recurring flight
    const pool = await getPool();
    const r    = pool.request().input('id', sql.Int, req.params.id);
    if (date) r.input('date', sql.NVarChar, date);

    // Với recurring flight: chỉ đếm ghế occupied đúng ngày đó (flight_date)
    // Với chuyến cụ thể: lấy tất cả ticket của flight_id
