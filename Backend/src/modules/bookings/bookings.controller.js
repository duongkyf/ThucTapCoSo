    ...
    for (const [i, p] of passengers.entries()) {
      try {
        ...
        const ticketRes = await pool.request()
          ...
          .input('fid',  sql.Int,      flight_id)
          .input('sid',  sql.Int,      p.seat_id ? Number(p.seat_id) : null)
          ...
          .input('fd',   sql.Date,     p.flight_date ? new Date(p.flight_date) : null)
          .query(`
            INSERT INTO dbo.Tickets (booking_id, flight_id, seat_id, passenger_name, passenger_type, identity_card, ticket_price, class, flight_date)
            OUTPUT INSERTED.ticket_id
            VALUES (@bid, @fid, @sid, @pn, @pt, @ic, @tp, @cls, @fd)
          `);
    ...
    const r = await pool.request()
      ...
      .query(`
        SELECT TOP 1 b.booking_id, b.booking_ref, b.status,
          f.flight_id, f.flight_code, f.departure_time, f.aircraft_id,
          src.city AS origin_city, src.airport_id AS origin_iata,
          dst.city AS dest_city,   dst.airport_id AS dest_iata,
...
const checkin = async (req, res) => {
  try {
    const { booking_ref, seat_code } = req.body;
    if (!booking_ref)
      return res.status(400).json({ success: false, message: 'Vui lòng nhập mã đặt chỗ' });
    ...
    if (['Đã hủy', 'Chờ hủy'].includes(r.recordset[0].status))
      return res.status(400).json({ message: 'Đặt vé không thể check-in' });

    // Resolve seat_id từ seat_code nếu được cung cấp
    let new_seat_id = null;
    if (seat_code) {
      const seatRes = await pool.request()
        .input('aircraft_id', sql.Int,      r.recordset[0].aircraft_id)
        .input('seat_code',   sql.NVarChar, seat_code.toUpperCase())
        .query(`SELECT seat_id FROM dbo.SeatMaps WHERE aircraft_id = @aircraft_id AND seat_code = @seat_code`);
      new_seat_id = seatRes.recordset[0]?.seat_id || null;
    }

    // Cập nhật ticket: status + seat_id
    await pool.request()
      .input('tid', sql.Int, r.recordset[0].ticket_id)
      .input('sid', sql.Int, new_seat_id)
      .query(`
        UPDATE dbo.Tickets
        SET status  = N'Đã check-in',
            seat_id = COALESCE(@sid, seat_id)
        WHERE ticket_id = @tid
      `);
...
module.exports = { create, getMyBookings, getById, cancel, requestCancel, lookup, checkin };